...
@section('title')
{{ translate('Add WooCommerce Store') }}
@endsection
...
<div class="row">
    <div class="col-md-12">
        <div class="align-items-center border-bottom2 d-flex flex-wrap gap-10 justify-content-between mb-4 pb-3">
            <h4><i class="icofont-plus"></i> {{ translate('Add WooCommerce Store') }}</h4>
    ...
    <div class="col-md-8 offset-md-2">
        <div class="card mb-30">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.dropshipping.woocommerce.store') }}" method="POST" id="woocommerceForm">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" class="form-control" id="store_url" name="store_url" value="{{ old('store_url') }}" required placeholder="https://yourstore.com">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="consumer_key" name="consumer_key" value="{{ old('consumer_key') }}" required>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="password" class="form-control" id="consumer_secret" name="consumer_secret" value="{{ old('consumer_secret') }}" required>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <button type="submit" class="btn btn-primary btn-block">
                                <i class="icofont-save"></i> {{ translate('Save Configuration') }}
                            </button>
                        </div>
                    </div>
                </form>
        ...
        $.post('{{ route("admin.dropshipping.woocommerce.test") }}', {
            ...
            .done(function(response) {
                ...
                if (response.success) {
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h6 class="card-title">${response.store_info.name || 'WooCommerce Store'}</h6>
                            <div class="row text-left">
                                <div class="col-6">
                                    <strong>{{ translate('Version') }}:</strong><br>
                                    <span class="text-muted">${response.store_info.version || 'N/A'}</span>
                                </div>
                                <div class="col-6">
                                    <strong>{{ translate('Products') }}:</strong><br>
                                    <span class="text-muted">${response.store_info.products_count || 0}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                `);
                    ...
                    $('#testSuccess').hide();
                    $('#testError').show();
                    $('#errorDetails').html(response.message || '{{ translate("Unknown error occurred") }}');
                }
            })
            .fail(function(xhr) {
                $('#testLoader').hide();
                ...
                $('#errorDetails').html(errorMsg);
            });
    }

    // Form validation
    $('#woocommerceForm').on('submit', function(e) {
        const storeUrl = $('#store_url').val();
        if (storeUrl && !storeUrl.startsWith('http://') && !storeUrl.startsWith('https://')) {
            e.preventDefault();
            alert('{{ translate("Store URL must start with http:// or https://") }}');
            $('#store_url').focus();
            return false;
        }
    });
